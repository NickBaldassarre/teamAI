from __future__ import annotations

import os
from pathlib import Path
from time import perf_counter
from typing import Any

from ..schemas import CodexHandoffPayload
from .bridge_base import (
    AgentBridge,
    BridgeExecutionResult,
    BridgeModelResponse,
    VerifiedBridgeExecutionResult,
    build_rate_limit_state,
    extract_headers,
    extract_usage_counts,
)
from .codex_bridge import _extract_response_text

DEFAULT_GROK_MODEL = "grok-4-1-fast-reasoning"
DEFAULT_GROK_PAYLOAD_FILE = ".teamai/codex_payload.json"
DEFAULT_GROK_PATCH_FILE = ".teamai/grok_solution.patch"
DEFAULT_GROK_FAILURE_CONTEXT_FILE = ".teamai/failure_context.log"

GrokHandoffExecutionResult = BridgeExecutionResult
VerifiedGrokHandoffExecutionResult = VerifiedBridgeExecutionResult


class GrokBridge(AgentBridge):
    engine = "grok"
    default_model = DEFAULT_GROK_MODEL

    def _request_patch(
        self,
        *,
        prompt: str,
        model: str,
        payload: CodexHandoffPayload,
        payload_path: Path,
        project_root: Path,
    ) -> BridgeModelResponse:
        client = _create_xai_client()
        started = perf_counter()
        response = client.responses.create(
            model=model,
            input=[
                {"role": "user", "content": prompt},
            ],
        )
        prompt_tokens, completion_tokens, total_tokens = extract_usage_counts(getattr(response, "usage", None))
        return BridgeModelResponse(
            patch_text=_extract_response_text(response),
            prompt_tokens=prompt_tokens,
            completion_tokens=completion_tokens,
            total_tokens=total_tokens,
            latency_ms=(perf_counter() - started) * 1000.0,
            rate_limit_state=build_rate_limit_state(
                provider="xai",
                model_id=model,
                prompt_tokens=prompt_tokens,
                completion_tokens=completion_tokens,
                total_tokens=total_tokens,
                headers=extract_headers(response),
                source="api_headers",
            ),
        )


def execute_grok_handoff(
    *,
    project_root: Path,
    payload_file: str | Path = DEFAULT_GROK_PAYLOAD_FILE,
    patch_file: str | Path = DEFAULT_GROK_PATCH_FILE,
    model: str | None = None,
) -> GrokHandoffExecutionResult:
    return GrokBridge().execute(
        project_root=project_root,
        payload_file=payload_file,
        patch_file=patch_file,
        model=model or os.getenv("TEAMAI_GROK_MODEL", "").strip() or DEFAULT_GROK_MODEL,
    )


def execute_verified_grok_handoff(
    *,
    project_root: Path,
    payload_file: str | Path = DEFAULT_GROK_PAYLOAD_FILE,
    patch_file: str | Path = DEFAULT_GROK_PATCH_FILE,
    failure_context_file: str | Path = DEFAULT_GROK_FAILURE_CONTEXT_FILE,
    model: str | None = None,
) -> VerifiedGrokHandoffExecutionResult:
    return GrokBridge().execute_verified(
        project_root=project_root,
        payload_file=payload_file,
        patch_file=patch_file,
        failure_context_file=failure_context_file,
        model=model or os.getenv("TEAMAI_GROK_MODEL", "").strip() or DEFAULT_GROK_MODEL,
    )


def _create_xai_client() -> Any:
    api_key = os.getenv("XAI_API_KEY", "").strip() or os.getenv("GROK_API_KEY", "").strip()
    if not api_key:
        raise RuntimeError("XAI_API_KEY is not set. Export it before running `teamai execute-handoff --engine grok`.")

    try:
        from openai import OpenAI
    except ImportError as exc:
        raise RuntimeError(
            "The OpenAI SDK is not installed. Install the project dependencies again so `openai` is available."
        ) from exc

    base_url = os.getenv("XAI_BASE_URL", "").strip() or "https://api.x.ai/v1"
    return OpenAI(api_key=api_key, base_url=base_url)
